...
struct TokenStoreSecurityTests {
    ...
    func restrictedFileStoreSetsPOSIX0600Permissions() async throws {
        ...
        #expect(permissions?.int16Value == 0o600, "Token file must have 0600 permissions (owner read/write only)")
    ...
    func restrictedFileStoreSetsDirectoryPermissions() async throws {
        ...
        #expect(permissions?.int16Value == 0o700, "Token directory must have 0700 permissions (owner access only)")
    ...
    func tokenStoreFactoryReturnsKeychainOnApple() {
            ...
            #expect(store is KeychainTokenStore, "Default store should be KeychainTokenStore on Apple platforms")
        #else
            Issue.record("Test only valid on Apple platforms with Security framework")
        #endif
    }
    ...
    func tokenStoreFactoryReturnsRestrictedFileOnNonApple() {
            ...
            #expect(store is RestrictedFileTokenStore, "Default store should be RestrictedFileTokenStore on non-Apple platforms")
        #else
    ...
    func tokensNotStoredInPlaintext() async throws {
        ...
        #expect(fileContent.contains("access_token") || fileContent.contains("accessToken"), 
    ...
    func inMemoryStoreDoesNotPersistToDisk() async throws {
        ...
        if let documentsURL {
            let documentsContents = try? FileManager.default.contentsOfDirectory(atPath: documentsURL.path)
            let hasSpotifyTokenFiles = documentsContents?.contains(where: { $0.contains("spotify") && $0.contains("token") }) ?? false
            #expect(!hasSpotifyTokenFiles,
        ...
        let tokenFiles = tempContents?.filter { $0.contains("spotify") && $0.contains("token") && !$0.contains("test") } ?? []
    ...
    func restrictedFileStoreSecurelyDeletesTokens() async throws {
        ...
        let tokenURL = tempDir.appendingPathComponent("test_tokens.json")
        #expect(FileManager.default.fileExists(atPath: tokenURL.path), "Token file should exist after save")
        
        try await store.clear()
        
        #expect(!FileManager.default.fileExists(atPath: tokenURL.path), "Token file should be deleted after clear")
    ...
    func keychainStoreSecurelyDeletesTokens() async throws {
        let testService = "com.spotifywebapi.security.test.\(UUID().uuidString)"
