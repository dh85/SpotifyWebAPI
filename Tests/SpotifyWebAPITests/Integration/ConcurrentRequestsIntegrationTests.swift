...
struct ConcurrentRequestsIntegrationTests {
    ...
    func multipleConcurrentAPICallsCompleteSuccessfully() async throws {
        ...
        try await server.withRunningServer { info in
            ...
            let (p1, p2, pl1, pl2, all) = try await (profile1, profile2, playlists1, playlists2, allPlaylists)
    ...
    private func countStreamItems(client: SpotifyClient<UserAuthCapability>, maxItems: Int) async throws -> Int {
        var count = 0
