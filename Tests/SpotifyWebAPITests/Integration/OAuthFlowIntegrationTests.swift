...
struct OAuthFlowIntegrationTests {
    ...
    func pkceFlowWithS256CompletesSuccessfully() async throws {
        ...
        try await server.withRunningServer { info in
            ...
            let authorizeURL = "\(info.authorizeEndpoint.absoluteString)?client_id=test-pkce-client&redirect_uri=\(redirectURI)&state=\(state)&response_type=code&code_challenge=\(codeChallenge)&code_challenge_method=S256&scope=user-read-email"
    ...
    func pkceFlowWithPlainChallenge() async throws {
        ...
        try await server.withRunningServer { info in
            ...
            let authorizeURL = "\(info.authorizeEndpoint.absoluteString)?client_id=test-client&redirect_uri=\(redirectURI)&state=\(state)&response_type=code&code_challenge=\(codeChallenge)&code_challenge_method=plain"
    ...
    func pkceFlowFailsWithInvalidVerifier() async throws {
        ...
        try await server.withRunningServer { info in
            ...
            let authorizeURL = "\(info.authorizeEndpoint.absoluteString)?client_id=test-client&redirect_uri=\(redirectURI)&state=\(state)&response_type=code&code_challenge=\(codeChallenge)&code_challenge_method=S256"
    ...
    func standardAuthorizationCodeFlow() async throws {
        ...
        try await server.withRunningServer { info in
            ...
            let authorizeURL = "\(info.authorizeEndpoint.absoluteString)?client_id=standard-client&redirect_uri=\(redirectURI)&state=\(state)&response_type=code&scope=playlist-modify-private"
    ...
    func authCodeFlowFailsWithInvalidCredentials() async throws {
        ...
        try await server.withRunningServer { info in
            ...
            let authorizeURL = "\(info.authorizeEndpoint.absoluteString)?client_id=valid-client&redirect_uri=\(redirectURI)&state=\(state)&response_type=code"
    ...
    func authFlowFailsWithMismatchedRedirectURI() async throws {
        ...
        try await server.withRunningServer { info in
            ...
            let authorizeURL = "\(info.authorizeEndpoint.absoluteString)?client_id=test-client&redirect_uri=\(redirectURI)&state=\(state)&response_type=code"
    ...
    func oauthStateParameterValidation() async throws {
        ...
        try await server.withRunningServer { info in
            ...
            let noStateURL = "\(info.authorizeEndpoint.absoluteString)?client_id=test-client&redirect_uri=\(redirectURI)&response_type=code"
    ...
    private func simulateAuthorization(url: String, expectedState: String?) async throws -> String {
        ...
        let authorizeURL = URL(string: "http://\(host):\(port)\(urlComponents.path)?\(urlComponents.query ?? "")")!
        ...
        guard let location = httpResponse.value(forHTTPHeaderField: "Location"),
              let redirectURL = URLComponents(string: location) else {
